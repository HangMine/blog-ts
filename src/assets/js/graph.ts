...
const Afterware = new ApolloLink((operation, forward: any) => {
  return forward(operation).map((response: any) => {
    ...
    if (result.msg) {
      result.code === 'success' ? message.success(result.msg) : message.error(result.msg)
    }
    return response
  })
})
...
const httpLink = new HttpLink({
  uri: 'http://localhost:8888/graphql', // 配置请求url 
})
...
const _useMutation = (gql: any) => {
  return useMutation(gql, {
    onCompleted(res) {
      ...
      const data = res[key];
      if (data.msg) {
        data.code === 'success' ? message.success(data.msg) : message.error(data.msg)
      }
    }
  })
}
