...
const AddArticle: FC<AddArticle> = ({ articleType }) => {
  ...
  const addArticle = () => {
    addArticleMutate({
      variables: {
        article: { title: "新建6", type: "2", content: "内容1", user: "7" }
      },
      refetchQueries: [
        {
          query: GET_ARTICLES
  ...
  const operateList = operates.map((item, i) => (
    <li key={i}>
      <Icon type={item.icon} />
      {item.name}
    </li>
  ...
  return (
    <HideScroll maxHeight="100vh">
      <div className="add-article">
        <div className="add-btn" onClick={addArticle}>
